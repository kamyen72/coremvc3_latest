...
namespace CoreMVC2.Controllers
{
    ...
    public class APIController : ControllerBase
    {
        ...
        [EnableCors("AllowAll")]
        [Route("GetOverallByUserV2")]
        [HttpPost]
        public IActionResult GetOverallByUserV2([FromBody] OverallByUserNameInput model)
        {
            string StartDate = model.StartDate;
            string EndDate = model.EndDate;
            string BetType = model.BetType;
            string UserName = model.UserName;
            string CurrentPeriod = model.CurrentPeriod;
            string Level2_ID = model.Level2_ID;
            ...
            //{
                ...
                sql += "SELECT ";
                sql += "CurrentPeriod as [Bill_No_Ticket] ";
                sql += ", ShowResultDate as [Openning_Time] ";
                sql += ", TOver = SUM(TOver) ";
                sql += ", TotalWinLose = SUM(MemberWL) ";
                sql += "FROM rt_MPlayer ";
                sql += "where LotteryTypeName = '@dbBetType' ";
                sql += "and UserName = '@dbAccount' ";
                sql += "and UpdateDate between @date1 and @date2 ";
                sql += "GROUP BY ";
                sql += "CurrentPeriod ";
                sql += ", ShowResultDate ";

                sql2 = sql.Replace("@dbStartDate", StartDate)
                .Replace("@dbEndDate", EndDate)
                .Replace("@dbAccount", UserName)
                .Replace("@dbBetType", BetType);
            //}
            ...
            //    sql += ",[MRec] ";
            //    sql += ",[TOver] ";
            //    sql += ",[MemberWL] as TotalWinLose ";
            //    sql += "FROM [dbo].[rt_Level3] ";
            //    sql += "where Level2_ID = '" + Level2_ID + "' ";

            //    sql2 = sql;
            //}

            SqlConnection connection = new SqlConnection(db_report.conn_str);
            DataTable myDataRows = new DataTable();
            SqlCommand command = new SqlCommand(sql2, connection);
            command.CommandTimeout = 600; // 5 minutes (60 seconds X 5)
            SqlDataAdapter adapter = new SqlDataAdapter(command);
            ...
            for (int i = 0; i < maxrows; i++)
            {
                ...
                OverallByUserName dat = new OverallByUserName();
                dat.TicketNo = row["Bill_No_Ticket"].ToString();
                dat.OpeningTime = DateTime.Parse( row["Openning_Time"].ToString() ).ToString("yyyy-MM-dd HH:mm:ss.fff");
                dat.TurnOver = decimal.Parse(row["TOver"].ToString());
                dat.TotalWinLose = decimal.Parse(row["TotalWinLose"].ToString());
        ...
        [EnableCors("AllowAll")]
        [Route("DrillOverall_Level4b")]
        [HttpPost]
        public IActionResult DrillOverall_Level4b([FromBody] OverallWith3ParamsInput model)
        {
            string StartDate = model.StartDate;
            string EndDate = model.EndDate;
            string BetType = model.BetType;
            ...
            string sql = "";
            sql += "select ";
            sql += "LotteryTypeName as [Bet_Type] ";
            sql += ", CurrentPeriod as [Bill_No_Ticket] ";
            sql += ", CreateDate as [BetTime] ";
            sql += ", concat('[', FamilyBigID, '][', SelectedNums, ']') as [Bet] ";
            sql += ", BetAmount = sum(BetAmount) ";
            sql += ", WL = sum(MemberWL) ";
            sql += "from rt_MPlayer ";
            sql += "where UpdateDate >= '@dbDateS' and UpdateDate <= '@dbDateE' ";
            sql += "and UserName = '@dbAccount' ";
            sql += "and CurrentPeriod = '@dbTicketNo' ";
            sql += "and LotteryTypeName = '@dbBetType' ";
            sql += "group by ";
            sql += "LotteryTypeName ";
            sql += ", CurrentPeriod ";
            sql += ", CreateDate ";
            sql += ", concat('[', FamilyBigID, '][', SelectedNums, ']') ";
            sql += "order by  ";
            sql += "LotteryTypeName ";
            sql += ", CurrentPeriod ";
            sql += ", CreateDate ";
            ...
            for (int i = 0; i < maxrows; i++)
            {
                ...
                OverallLevel4Data dat = new OverallLevel4Data();
                dat.BetType = row["Bet_Type"].ToString();
                dat.BetTime = DateTime.Parse( row["BetTime"].ToString() ).ToString("yyyy-MM-dd HH:mm:ss.fff");
                dat.Bet = row["Bet"].ToString();
                dat.Bill_No_Ticket = row["Bill_No_Ticket"].ToString();
                dat.BetAmount = decimal.Parse(row["BetAmount"].ToString());
                dat.WinLose = decimal.Parse(row["WL"].ToString());
